...
def main(args_eval, resume_preempt=False):
    ...
    # Initialize model
    encoder, predictor = init_frozen_models(
...
def init_frozen_models(
    ...
    encoder = vit_encoder.__dict__[model_name](
        img_size=crop_size,
        patch_size=patch_size,
    ...
    encoder.to(device)
    predictor.to(device)
    encoder = load_pretrained(encoder=encoder, pretrained_model_fpath=pretrained, checkpoint_key=checkpoint_key)
...
def load_pretrained(model, pretrained_model_fpath, checkpoint_key):
    logger.info(f"Loading pretrained model from {pretrained_model_fpath}")
    checkpoint = torch.load(pretrained_model_fpath, map_location="cpu")
